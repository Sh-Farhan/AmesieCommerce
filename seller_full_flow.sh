...
# Product fields from your Swagger schema
...
CATEGORY_ID="${CATEGORY_ID:-1}"             # must exist
# ========================================================================
...
if [[ "$REG_STATUS" =~ ^20[01]$ ]]; then
  ...
  say "Registration returned $REG_STATUS (likely email exists). Continuing."
else
  cat /tmp/reg.out
  fail "Unexpected register status: $REG_STATUS"
fi

# 2) Login EXACTLY like Swagger’s OAuth2PasswordRequestForm
...
[[ -z "$TOKEN" ]] && { echo "$LOGIN_RAW"; fail "Login failed. No access_token."; }

# sanity check the JWT
...
# 3) Verify identity and fetch seller_id
...
# 4) Create product (match your backend schema)
...
# 5) List first page of seller products
